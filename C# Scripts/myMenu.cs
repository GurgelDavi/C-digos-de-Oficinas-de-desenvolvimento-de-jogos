...
public class myMenu : MonoBehaviour {
	...
	void OnGUI()
	{
		if (Network.peerType ==  NetworkPeerType.Disconnected)
		{
			...
			if (Network.peerType  ==  NetworkPeerType.Server && !GameStart)
			{
				...
				{
					//networkView.RPC("AskInstantiate",RPCMode.All,myPlayer);
	...
	{
		if (Network.isServer)
			networkView.RPC ("GameStatus", RPCMode.All);
	}
	[RPC]
	void GameStatus(bool gameStart)
	{
		this.GameStart = true;
	}
	[RPC]
	...
	{
		if (Network.isServer)
			networkView.RPC ("PlayersPositionUpdate", RPCMode.All);
	}
	[RPC]
	void PlayersPositionUpdate(Vector3 _Player1Pos, Vector3 _Player2Pos) 
	{
		this.Player1.myBoat.transform.position = _Player1Pos;
		this.Player2.myBoat.transform.position = _Player2Pos;
	}
	[RPC]
	void AskInstantiate(int _player)
	{
		if (Network.isServer)
		{
			networkView.RPC("InstantiateBoat",RPCMode.All,_player);
		}
	}
	[RPC]
	...
	{
		if (Network.isServer) {
			Debug.Log ("Asking ready" + _myPlayer);
			this.playersReady [_myPlayer - 1] = _myPlayer;
			Debug.Log ("Player ready: " + _myPlayer);
		}
